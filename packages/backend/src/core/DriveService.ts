...
export class DriveService {
	...
	public async reCacheFile(fileId: MiDriveFile['id']) {
		...
		if (!file || !file.uri || !file.isLink || (file.isSensitive && !this.meta.cacheRemoteSensitiveFiles)) {
			const reason = !file ? 'file is null' : (!file.uri ? 'file uri is null' : (file.isLink ? 'file is not link' : ((file.isSensitive && !this.meta.cacheRemoteSensitiveFiles) ? 'Remote sensitive media caching is disabled' : `??? file: ${JSON.stringify(file)}`)));
