...
export class AvatarDecorationService implements OnApplicationShutdown {
	...
	public async remoteUserUpdate(user: MiUser) {
		...
		const avatarDecorations = userData.avatarDecorations ?? undefined;

		if (!avatarDecorations || avatarDecorations.length === 0) {
		...
		const updates = {} as Partial<MiUser>;
		updates.avatarDecorations = [];
		for (const avatarDecoration of avatarDecorations) {
			let name;
			let description;
